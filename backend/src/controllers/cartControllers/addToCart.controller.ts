...
export const addToCartFunction = async (req: Request, res: Response) => {
    try {
        ...
        const alreadyCreatedCart = await CartModel.findOne({
            user: userMakingRequest._id,
        });
        if (alreadyCreatedCart) {
            const productToAddInAlreadyExistingCart: any = {
            ...
            res.status(200).json(
                new ApiResponse(200, alreadyCreatedCart, "Items added to cart")
            );
        } else {
            //if it doesnot
            ...
            res.status(200).json(
                new ApiResponse(200, cart, "Items added to cart")
            );
        }
    } catch (err: any) {
        console.error(err);
