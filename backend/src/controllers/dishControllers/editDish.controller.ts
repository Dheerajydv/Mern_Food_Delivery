...
export const editDishFunction = async (req: Request, res: Response) => {
    try {
        ...
        const dishWithNewNameAlreadyExists = await DishModel.findOne({newName})
        if(dishWithNewNameAlreadyExists){
            throw new ApiError(400, "Dish with new name already exists")
        }
        // update the picture
        ...
        if(newDishPictureLocalPath){
            ...
            const newDishPicture = await uploadOnCloudinary(newDishPictureLocalPath);
            if(!newDishPicture){
        ...
        const dishFoundAndUpdated = await DishModel.findByIdAndUpdate(req.params.id, updatedData, {new: true})
        if(!dishFoundAndUpdated){
            throw new ApiError(500, "Dish not found")
        }
        
        res.status(200).json(new ApiResponse(200, dishFoundAndUpdated, "Dish updated sucessfully"))
    } catch (err: any) {
        console.error(err);
