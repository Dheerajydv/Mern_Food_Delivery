...
export const getSearchedDishFunction = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json(
            new ApiResponse(200, searchedDish, "Found your dish")
        );
    } catch (err: any) {
        console.error(err);
...
export const getDishBySearchedCategoryFunction = async (
    req: Request,
    res: Response
) => {
    try {
        ...
        const allDishesOfSearchedCategory = await DishModel.find({
        ...
        res.status(200).json(
            new ApiResponse(
                200,
                allDishesOfSearchedCategory,
                `All ${categoryOfDishToSearch} fetched`
            )
        );
    } catch (err: any) {
        console.error(err);
